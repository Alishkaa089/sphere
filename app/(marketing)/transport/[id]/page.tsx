...
export default function TransportDetails({ params }: { params: Promise<{ id: string }> }) {
  const unwrappedParams = use(params);
  const { t } = useLanguage();
  const [reservationDays, setReservationDays] = useState(1);
  
  const vehicles = generateVehicles();
  const vehicle = vehicles.find(v => v.id === unwrappedParams.id) || vehicles[0];

  const priceVal = parseInt(vehicle.price.replace(/,/g, ''));
      ...
      <div className="relative h-[65vh] md:h-[80vh] w-full">
        ...
        <div className="absolute bottom-12 left-6 md:left-24 max-w-4xl pr-6">
          ...
          <motion.div 
            ...
            className="flex items-center gap-2 text-indigo-300 font-bold text-lg md:text-xl drop-shadow-md"
          >
            <MapPin className="w-5 h-5" /> {vehicle.loc}, {vehicle.country}
          </motion.div>
        </div>
      </div>
        ...
        <div className="lg:col-span-2">
            ...
            >
                <div className="bg-white/5 border border-white/10 rounded-2xl p-6 flex flex-col items-center justify-center text-center">
                    <Battery className="w-8 h-8 text-indigo-400 mb-3" />
                ...
                <div className="bg-white/5 border border-white/10 rounded-2xl p-6 flex flex-col items-center justify-center text-center">
                    <Gauge className="w-8 h-8 text-indigo-400 mb-3" />
                    <span className="text-3xl font-black text-white mb-1">3.1s</span>
                    <span className="text-xs font-bold text-slate-500 uppercase tracking-widest">{t.trans_range} (0-100)</span>
                </div>
                <div className="bg-white/5 border border-white/10 rounded-2xl p-6 flex flex-col items-center justify-center text-center">
                    <Zap className="w-8 h-8 text-indigo-400 mb-3" />
                    <span className="text-3xl font-black text-white mb-1">650+</span>
            ...
            <div className="mb-16">
                ...
                <div className="w-full h-[400px] bg-zinc-900 rounded-3xl overflow-hidden relative border border-white/10 flex items-center justify-center">
                    ...
                    <div className="relative z-10 bg-black/60 backdrop-blur-xl border border-white/20 p-6 rounded-2xl text-center shadow-2xl">
                        ...
                        <h4 className="text-xl font-black text-white">{vehicle.loc}</h4>
