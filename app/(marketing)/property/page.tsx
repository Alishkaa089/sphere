...
export default function PropertyPage() {
  ...
  useEffect(() => {
    setAllProperties(generateProperties());
  }, []);
  ...
  const handleCountrySelect = (ctry: string) => {
    ...
    setSelectedCity("Bütün Şəhərlər"); // Reset city when changing country
  ...
  const filteredProperties = ALL_PROPERTIES.filter(prop => {
    ...
    const matchesCity = selectedCity === "Bütün Şəhərlər" || prop.loc === selectedCity;
    ...
    const matchesSearch = prop.title.toLowerCase().includes(searchQuery.toLowerCase()) || 
                          prop.loc.toLowerCase().includes(searchQuery.toLowerCase());
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 pt-32 pb-24 relative z-10">
        ...
        <div className="max-w-4xl mb-12 sm:mb-20 pt-10">
          ...
          >
            ...
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-300 to-indigo-300">{t.hero_title_part2}</span>
          </motion.h1>
            ...
            <div className="relative w-full md:w-56 z-50">
              ...
              >
                <div className="flex items-center gap-2 truncate">
                  ...
                  <span className="truncate text-sm">{selectedCountry === "Bütün Ölkələr" ? t.all_countries : selectedCountry}</span>
                </div>
            ...
            <div className="relative w-full md:w-56 z-40">
              ...
              >
                <div className="flex items-center gap-2 truncate">
                  ...
                  <span className="truncate text-sm">{selectedCountry === "Bütün Ölkələr" ? t.select_country_first : (selectedCity === "Bütün Şəhərlər" ? t.all_cities : selectedCity)}</span>
                </div>
              ...
              <button 
                ...
                className={`whitespace-nowrap px-6 py-2.5 rounded-full text-sm font-bold transition-all ${
                  selectedCategory === cat 
                    ...
                    : "bg-zinc-900 border border-zinc-800 text-slate-400 hover:border-blue-500 hover:text-blue-400"
                }`}
              >
                {cat === "Bütün Mülklər" ? t.all_properties : cat === "Premium Villalar" ? t.premium_villas : cat === "Penthaus" ? t.penthouse : cat === "Ağıllı Evlər" ? t.smart_homes : cat === "Dəniz Mənzərəli" ? t.sea_view : cat}
              </button>
            ))}
          </div>
                  ...
                  <Card className="h-full overflow-hidden border-0 bg-transparent flex flex-col relative rounded-3xl">
                    ...
                    <div className="relative h-[300px] w-full rounded-3xl overflow-hidden shadow-lg border border-white/10">
                      ...
                      <div className="absolute top-4 left-4 right-4 flex justify-between items-start">
                        <div className="flex flex-col items-start gap-2">
                          <div className={`backdrop-blur-md border border-white/30 text-white text-xs font-bold px-3 py-1.5 rounded-full shadow-lg ${prop.status === "Satış" ? "bg-emerald-500/50" : "bg-blue-500/50"}`}>
                            {prop.status === "Satış" ? t.for_sale : t.for_rent}
                          </div>
                          <div className="bg-black/50 backdrop-blur-md border border-white/10 text-white text-xs font-bold px-3 py-1.5 rounded-full shadow-lg flex items-center gap-1">
                            <Globe className="w-3 h-3 text-blue-400" /> {prop.country}
                          </div>
                        </div>
                    ...
                    <CardContent className="px-1 py-5 flex-1 flex flex-col justify-between">
                      <div className="flex justify-between text-slate-400 mb-5 text-sm font-semibold">
                        <div className="flex items-center gap-1.5 bg-zinc-900 border border-white/5 px-2.5 py-1.5 rounded-lg"><BedDouble className="w-4 h-4 text-blue-500"/> {prop.beds}</div>
                        ...
                        <div className="flex items-center gap-1.5 bg-zinc-900 border border-white/5 px-2.5 py-1.5 rounded-lg"><Square className="w-4 h-4 text-indigo-500"/> {prop.area}m²</div>
                      </div>
                      
                      <div className="flex items-end justify-between border-t border-zinc-800/80 pt-4">
                        <div>
                          <p className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">
                            {prop.status === "Satış" ? t.sale_price : t.monthly_rent}
                          </p>
