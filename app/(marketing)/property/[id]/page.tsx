...
import { ArrowLeft, MapPin, Building2, Star, CheckCircle, ShieldCheck, Flame, Square, BedDouble, Bath } from "lucide-react";
import { generateProperties } from "@/utils/mockData";
...
export default function PropertyDetails({ params }: { params: Promise<{ id: string }> }) {
  ...
  const property = properties.find(p => p.id === unwrappedParams.id) || properties[0];

  const SIMILAR_PROPERTIES = properties.filter(p => p.country === property.country && p.id !== property.id).slice(0, 3);

  const priceVal = parseInt(property.price.replace(/,/g, ''));
  const discountAmount = property.status === "Satış" ? 0 : priceVal * 0.1 * reservationDays;
  const isSale = property.status === "Satış";
  ...
  return (
    <div className="min-h-screen bg-[#0a0a09] text-white">
      ...
      <div className="fixed top-0 left-0 right-0 z-50 px-6 py-6 md:px-12 pointer-events-none flex justify-between items-center">
        <Link href="/property" className="pointer-events-auto w-12 h-12 bg-black/50 backdrop-blur-3xl border border-white/20 rounded-full flex items-center justify-center text-white hover:bg-white/20 transition-all focus:outline-none shadow-2xl">
          <ArrowLeft className="w-5 h-5" />
        </Link>
      </div>

      {/* Hero Visual Section */}
      <div className="relative h-[65vh] md:h-[80vh] w-full mt-0 md:mt-0">
        <motion.img 
        ...
        <div className="absolute bottom-12 left-6 md:left-24 max-w-4xl pr-6">
          ...
          >
             <span className={`px-4 py-1.5 rounded-full text-xs font-black uppercase tracking-widest text-white border border-white/20 backdrop-blur-md shadow-2xl ${isSale ? 'bg-emerald-500/80' : 'bg-blue-500/80'}`}>
             ...
             <span className="flex items-center gap-1.5 px-4 py-1.5 rounded-full text-xs font-black bg-amber-500/20 text-amber-400 border border-amber-500/20 backdrop-blur-md">
               <Star className="w-4 h-4 fill-amber-400" /> {property.rating}
             </span>
          </motion.div>
          <motion.h1 
          ...
          <motion.div 
            ...
            className="flex items-center gap-2 text-blue-300 font-bold text-lg md:text-xl drop-shadow-md"
          >
            <MapPin className="w-5 h-5" /> {property.loc}, {property.country}
          </motion.div>
        </div>
      </div>
        ...
        <div className="lg:col-span-2">
            ...
            >
                <div className="bg-white/5 border border-white/10 rounded-2xl p-6 flex flex-col items-center justify-center text-center">
                    ...
                    <span className="text-3xl font-black text-white mb-1">4</span>
                    <span className="text-xs font-bold text-slate-500 uppercase tracking-widest">{t.prop_beds}</span>
                </div>
                <div className="bg-white/5 border border-white/10 rounded-2xl p-6 flex flex-col items-center justify-center text-center">
                    <Bath className="w-8 h-8 text-blue-400 mb-3" />
                    <span className="text-3xl font-black text-white mb-1">5</span>
                    <span className="text-xs font-bold text-slate-500 uppercase tracking-widest">{t.prop_baths}</span>
                </div>
                <div className="bg-white/5 border border-white/10 rounded-2xl p-6 flex flex-col items-center justify-center text-center">
                    <Square className="w-8 h-8 text-blue-400 mb-3" />
                    <span className="text-3xl font-black text-white mb-1">850</span>
                    <span className="text-xs font-bold text-slate-500 uppercase tracking-widest">{t.prop_sqm}</span>
                </div>
            </motion.div>
            ...
            <div className="mb-16">
                ...
                <div className="prose prose-invert prose-lg text-slate-300 leading-relaxed font-medium">
                    <p className="mb-6">
                        {property.title} - {t.cat_prop_desc}
                    </p>
                    <div className="bg-indigo-900/20 border border-indigo-500/20 rounded-2xl p-6 mb-8 flex items-start gap-4">
            ...
            <div className="mb-16">
                ...
                <div className="w-full h-[400px] bg-zinc-900 rounded-3xl overflow-hidden relative border border-white/10 flex items-center justify-center">
                    ...
                    <div className="relative z-10 bg-black/60 backdrop-blur-xl border border-white/20 p-6 rounded-2xl text-center shadow-2xl">
                        ...
                        <h4 className="text-xl font-black text-white">{property.loc}</h4>
                        <p className="text-slate-400 font-bold">{property.country}</p>
                    </div>
                </div>
            </div>
            {/* Similar Options mapped safely via t bindings skipped for brevity assuming mockData loops dynamically */}
        </div>
