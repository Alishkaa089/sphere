    ...
    return (
      <div className="min-h-screen bg-[#0a0a09] flex flex-col items-center justify-center p-4">
        ...
        <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="text-4xl font-black text-white text-center mb-4">
          Təbriklər! Siz artıq PRO plandasınız.
        </motion.h1>
        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }} className="text-emerald-400 font-medium text-lg text-center max-w-sm">
          Sizin Valorum PRO abunəliyiniz 30 gün müddətinə aktivləşdirildi. Bütün limitlər genişləndi.
        </motion.p>
      </div>
    );
  }
      ...
      <div className="max-w-5xl w-full grid grid-cols-1 md:grid-cols-2 gap-8 z-10">
          ...
          <form onSubmit={handlePay} className="space-y-6">
            <div className="space-y-4">
              <div>
                ...
                <input required type="text" autoComplete="off" value={cardName} onChange={handleNameChange} placeholder="John Doe" className="w-full bg-black/50 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-[#006B8A] transition-colors placeholder:text-slate-600" />
              </div>
              <div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input required type="text" autoComplete="off" value={expiry} onChange={handleExpiryChange} placeholder="AA/İİ" className="w-full bg-black/50 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-[#006B8A] transition-colors placeholder:text-slate-600" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase mb-2">CVC/CVV</label>
                  <div className="relative">
                    <input required autoComplete="off" type={showCvv ? "text" : "password"} value={cvv} onChange={handleCvvChange} placeholder="•••" className="w-full bg-black/50 border border-white/10 rounded-xl pl-4 pr-12 py-3 text-white focus:outline-none focus:border-[#006B8A] transition-colors placeholder:text-slate-600" />
            ...
            <button disabled={isProcessing || !isFormValid()} type="submit" className="w-full mt-8 py-4 rounded-xl font-black text-lg transition-all flex items-center justify-center gap-2 bg-gradient-to-r from-[#004E64] to-[#006B8A] hover:from-[#006B8A] hover:to-[#00A3CC] text-white shadow-xl shadow-[#004E64]/30 disabled:opacity-50 disabled:cursor-not-allowed">
              {isProcessing ? "İcra Edilir..." : "Ödə - $99"}
            </button>
        ...
        <div className="bg-zinc-900/60 backdrop-blur-2xl border border-white/10 rounded-3xl overflow-hidden shadow-2xl flex flex-col">
          <div className="relative h-48 w-full bg-gradient-to-bl from-[#006B8A] to-[#00394A] flex items-center justify-center">
            <div className="text-5xl font-black text-white" style={{ fontFamily: "'Grailga', serif" }}>PRO Plan</div>
          ...
          <div className="p-8 pt-0 flex-1 flex flex-col mt-[-20px] relative z-10">
            ...
            <h3 className="text-2xl font-black text-white mb-2">Valorum PRO Abunəliyi</h3>
            <p className="text-slate-400 font-medium mb-6 text-sm">Aylıq plan (30 gün)</p>
            
            <div className="bg-black/50 rounded-2xl p-5 border border-white/5 space-y-4 flex-1">
              <div className="flex justify-between items-center text-sm font-semibold">
                <span className="text-slate-400">Limit</span>
                <span className="text-white">50 elan</span>
              </div>
              <div className="flex justify-between items-center text-sm font-semibold">
