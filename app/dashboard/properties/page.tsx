      ...
      <div className="p-8 lg:p-12">
         <header className="flex flex-col md:flex-row md:items-center justify-between mb-12 gap-6">
            <div>
               <motion.h1 initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="text-4xl font-black text-white tracking-tight mb-2 flex items-center gap-3">
                 <Building className="w-8 h-8 text-amber-500" /> Bütün Mülklər
               </motion.h1>
               <motion.p initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="text-slate-400 font-medium">Bazada olan {items.length} əmlaka nəzarət edin.</motion.p>
            </div>
         ...
         <div className="bg-black/30 border border-white/5 rounded-3xl p-6 shadow-2xl overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full text-left whitespace-nowrap">
                 <thead>
                    <tr className="border-b border-white/10 text-slate-500">
                       ...
                       <th className="pb-4 font-black uppercase text-xs tracking-widest px-4">Başlıq</th>
                       ...
                       <th className="pb-4 font-black uppercase text-xs tracking-widest px-4">Kateqoriya</th>
                 ...
                 <tbody>
                    {filtered.map((item, idx) => (
                       <tr key={item.id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                          ...
                          <td className="py-4 px-4 font-black text-amber-400">${item.price}</td>
                          <td className="py-4 px-4">
              ...
              {filtered.length === 0 && (
                 <div className="text-center py-20 text-slate-500 font-bold">Heç bir mülk tapılmadı.</div>
              )}
            </div>
         </div>
      </div>
